...
class BaseHFEngine(TrainEngine):
    ...
    def prepare_mb_list(self, input_: TensorDict) -> MicroBatchList:
        ...
        if self.is_vision_model:
            assert (
                "pixel_values" in input_ and "image_grid_thw" in input_ or "multi_modal_input" in input_
        ...
        if is_qwen2_vl_model(self.model_config.model_type):
            ...
            if "multi_modal_input" in input_: # overwrite previous image_grid_thw/video_grid_thw
                multi_modal_input = input_["multi_modal_input"]
                image_grid_thw=None
                video_grid_thw=None
                image_grid_thw_list=[m["image_grid_thw"] for m in multi_modal_input if "image_grid_thw" in m]
                if image_grid_thw_list:
                    image_grid_thw=torch.cat(image_grid_thw_list)
                video_grid_thw_list = [m["video_grid_thw"] for m in multi_modal_input if "video_grid_thw" in m]
        ...
        for mb,padded_mb in zip(mb_list.mbs,mb_list.padded_mbs):
            ...
            padded_mb["cu_seqlens_q"] = padded_mb["cu_seqlens_k"] = padded_mb["cu_seqlens"]
            mb["use_cache"] = False
            ...
            if "multi_modal_input" in mb:
                image_grid_thw_list = [item["image_grid_thw"] for item in mb["multi_modal_input"] if "image_grid_thw" in item]
                if image_grid_thw_list:
                    mb["image_grid_thw"] = torch.cat(image_grid_thw_list, dim=0)
                    padded_mb["image_grid_thw"] = torch.cat(image_grid_thw_list, dim=0)
                pixel_values_list = [item["pixel_values"] for item in mb["multi_modal_input"] if "pixel_values" in item]
                if pixel_values_list:
                    mb["pixel_values"] = torch.cat(pixel_values_list, dim=0)
                    padded_mb["pixel_values"] = torch.cat(pixel_values_list, dim=0)
                video_grid_thw_list = [item["video_grid_thw"] for item in mb["multi_modal_input"] if "video_grid_thw" in item]
                if video_grid_thw_list:
