...
def run_evaluation(
    ...
    with stats_tracker.record_timing("eval"):
        ...
        def evaluate_fn():
            ...
            eval_avg_success = None
            per_tag_reward_totals: Dict[int, float] = {}
            per_tag_reward_counts: Dict[int, int] = {}
            per_tag_success_totals: Dict[int, float] = {}
            per_tag_success_counts: Dict[int, int] = {}
            ...
            if isinstance(results, dict):
                ...
                if "success" in results:
                    success_tensor = results["success"].float().view(-1).cpu()
                    if success_tensor.numel() > 0:
                        eval_avg_success = float(success_tensor.mean().item())
                if "tag_id" in results:
                    tag_tensor = results["tag_id"].long().view(-1).cpu()
                    if rewards_tensor is not None and tag_tensor.numel() == rewards_tensor.numel():
                        for reward, tag_id in zip(rewards_tensor.tolist(), tag_tensor.tolist()):
                            per_tag_reward_totals[tag_id] = per_tag_reward_totals.get(tag_id, 0.0) + reward
                            per_tag_reward_counts[tag_id] = per_tag_reward_counts.get(tag_id, 0) + 1
                    if success_tensor is not None and tag_tensor.numel() == success_tensor.numel():
                        for success, tag_id in zip(success_tensor.tolist(), tag_tensor.tolist()):
                            per_tag_success_totals[tag_id] = per_tag_success_totals.get(tag_id, 0.0) + success
                            per_tag_success_counts[tag_id] = per_tag_success_counts.get(tag_id, 0) + 1
            ...
            if eval_avg_reward is not None:
                ...
                for tag_id, total in per_tag_reward_totals.items():
                    count = per_tag_reward_counts.get(tag_id, 0)
                    if count > 0:
                        tag_key = f"tag_{tag_id}"
                        stats_tracker.scalar(**{f"eval_avg_reward/{tag_key}": total / count})
            if eval_avg_success is not None:
                stats_tracker.scalar(eval_avg_success=eval_avg_success)
                for tag_id, total in per_tag_success_totals.items():
                    count = per_tag_success_counts.get(tag_id, 0)
                    if count > 0:
                        tag_key = f"tag_{tag_id}"
                        stats_tracker.scalar(**{f"eval_avg_success/{tag_key}": total / count})
    ...
    for global_step in range(start_step, max_steps):
        ...
        if actor.is_data_parallel_head() and "tag_id" in batch:
            ...
            if tag_tensor.numel() > 0:
                unique_tag_list = torch.unique(tag_tensor).tolist()
                if "rewards" in batch:
                    rewards_tensor = batch["rewards"].detach().float().cpu().view(-1)
                    if rewards_tensor.numel() == tag_tensor.numel() and rewards_tensor.numel() > 0:
                        stats_tracker.scalar(train_avg_reward=float(rewards_tensor.mean().item()))
                        for tag_id in unique_tag_list:
                ...
                if "success" in batch:
                    success_tensor = batch["success"].detach().float().cpu().view(-1)
                    if success_tensor.numel() == tag_tensor.numel() and success_tensor.numel() > 0:
                        stats_tracker.scalar(train_avg_success=float(success_tensor.mean().item()))
                        for tag_id in unique_tag_list:
                            mask = tag_tensor == tag_id
                            if mask.any():
                                tag_success = success_tensor[mask].mean().item()
                                tag_key = f"tag_{tag_id}"
                                stats_tracker.scalar(**{f"train_avg_success/{tag_key}": float(tag_success)})
        dist.barrier(device_ids=[actor.device.index])
        current_platform.synchronize()
