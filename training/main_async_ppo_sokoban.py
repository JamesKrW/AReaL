...
from realhf.experiments.async_exp.async_sokoban_ppo import AsyncPPOSokobanConfig
from training.utils import run_experiment

@hydra.main(version_base=None, config_path="configs", config_name="async-ppo")
def main_ppo_sokoban(args):
    # Import logging here to avoid hydra logging overwrite
    import realhf.base.logging as logging

    logger = logging.getLogger("quickstart", "colored")

    # Overwrite the python dataclass configuration with yaml
    default_args = OmegaConf.structured(AsyncPPOSokobanConfig)
    args = OmegaConf.merge(default_args, args)
    args: AsyncPPOSokobanConfig = OmegaConf.to_object(args)
    ...
    kind_reminder("async-ppo-sokoban", logger, args)
...
if __name__ == "__main__":
    ...
    if args.help:
        ...
        print_config_help(AsyncPPOSokobanConfig())
        exit(0)
